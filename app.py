...
def criar_link_atendimento(protocolo):
    if not protocolo or str(protocolo) == "0" or str(protocolo).lower() == "nan": return None
    proto_str = str(protocolo).strip().replace('.0', '') # Remove decimal point if exists
                            ...
                            for resp in respostas:
                                ...
                                if id_pesquisa_str == ID_PESQUISA_V3:
                                    if "internet" in nome_lower:
                                        ...
                                        # continue # REMOVIDO PARA TESTE: Se quiser ignorar, descomente
                                        pass
...
# ==============================================================================
    ...
    with st.sidebar:
        ...
        # --- NOVO: UPLOADER DE ARQUIVOS ---
        st.markdown("---")
        uploaded_files = st.file_uploader("📂 Importar CSV/Excel (Extra)", accept_multiple_files=True, type=['csv', 'xlsx'])
        st.caption("Use isso para pesquisas que a API não retornou.")
        st.markdown("---")
    ...
    # Se já tiver pesquisas na lista OU arquivos carregados, mostra o botão de gerar
    if st.session_state["pesquisas_list"] or uploaded_files:
        ...
        if gerar:
            # 1. Baixa da API
            raw_data = []
            audit_results = {"Aceitas": set(), "Ignoradas": set()}
            ...
            # 2. Processa Arquivos Carregados (CSV/Excel)
            if uploaded_files:
                for u_file in uploaded_files:
                    try:
                        if u_file.name.endswith('.xlsx'):
                            df_temp = pd.read_excel(u_file)
                        else:
                            df_temp = pd.read_csv(u_file)
                        
                        # Limpa nomes de colunas
                        df_temp.columns = df_temp.columns.str.strip()
                        
                        # Mapeamento de colunas do CSV para o padrão interno
                        # Padrão esperado: nom_valor, nom_agente, dat_resposta, num_protocolo, conta_origem_id, nom_resposta
                        mapa = {
                            'Opção': 'nom_valor', 
                            'Agente': 'nom_agente', 
                            'Data': 'dat_resposta', 
                            'Protocolo': 'num_protocolo',
                            'Resposta': 'nom_resposta',
                            'Conta': 'conta_origem_id', # Usamos a string da conta direto como ID temporário
                            'Classificação': 'nom_pergunta' # Opcional
                        }
                        ...
                        # Converte data do CSV (Geralmente DD/MM/YYYY)
                        df_temp['dat_resposta'] = pd.to_datetime(df_temp['dat_resposta'], dayfirst=True, errors='coerce')
                        
                        # Converte para lista de dicionários e adiciona ao raw_data
                        csv_dicts = df_temp.to_dict(orient='records')
                        raw_data.extend(csv_dicts)
                        st.toast(f"Arquivo {u_file.name} processado: {len(csv_dicts)} registros.", icon="📂")
                        
                    except Exception as e:
                        st.error(f"Erro ao ler arquivo {u_file.name}: {e}")

            # 3. Processamento Final
            if not raw_data:
                st.warning("Nenhum dado encontrado (nem na API, nem nos arquivos).")
            else:
                ...
                if 'conta_origem_id' not in df: df['conta_origem_id'] = "Desconhecida"
                ...
                # Tratamento do Nome da Conta (Híbrido API/CSV)
                # Tenta mapear pelo ID, se não conseguir (ex: veio do CSV com nome direto), usa o próprio valor
                df['Nome_Conta'] = df['conta_origem_id'].map(CONTAS_FIXAS)
                df['Nome_Conta'] = df['Nome_Conta'].fillna(df['conta_origem_id']).fillna("Desconhecida")
